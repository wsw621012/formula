...
class ImageProcessor(object):
    ...
    def find_wall_angle(target, debug = False):
        ...
        left_y = max(_y)
        while left_y >= min(_y):
            if target[left_y, left_x] == 0:
                break
            left_y -= 1

        right_x = max(_x)
        right_y = max(_y)
        while right_y >= min(_y):
            if target[right_y, right_x] == 0:
                break
            right_y -= 1

        if left_x == 0 and left_y == (target.shape[0] - 1):
            while left_x < right_x:
                if target[left_y, left_x + 1] != 0:
                    break
                left_x += 1

        if right_x == (target.shape[1] - 1) and right_y == (target.shape[0] - 1):
            while right_x > left_x:
                if target[right_y, right_x - 1] != 0:
                    break
                right_x -= 1
    ...
    @staticmethod
    def get_steering_angle(im_gray):

        # color remains => red:76, black:0, white:255
        target = ImageProcessor._crop_gray(im_gray, 0.6, 1.0)
        b, r, w = ImageProcessor._color_rate(target)
        print("b:%.2f, r:%.2f, w:%.2f" %(b, r, w))
        if b > 0.9:
            return 180

        return ImageProcessor.find_red_angle(im_gray)
